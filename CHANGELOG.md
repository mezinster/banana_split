...
- **Camera permission only** (Android, #1, #25): `RECORD_AUDIO` and `WRITE_EXTERNAL_STORAGE` (≤ API 28) from `camera_android_camerax`, and `INTERNET`/`ACCESS_NETWORK_STATE` from Google `datatransport` (ML Kit telemetry, GitHub build only), are stripped at manifest merge. The app never records audio (`enableAudio: false` on every `CameraController`, asserted by a test for both scanner variants), writes captures to its cache directory, and makes no network calls of its own — so the OS now enforces "no server communication". Verified on a Pixel 8 Pro (Android 17): ML Kit scans normally and `datatransport` only logs a warning. Both builds now request `CAMERA` only.
