...
- **The bot copies the previous build entry, it does not regenerate from `templates/build-flutter.yml`.** The bot-authored 0.8.3 entry carries our app-specific `sed -i -e '/mobile_scanner/d'` and `mv shard_scanner.dart.fdroid` lines, which appear nowhere in that template — a template-generated entry could not contain them. Two consequences: the recipe cannot spontaneously acquire flags such as `--enforce-lockfile`, and conversely a recipe change never propagates on its own. **An MR is required only when the recipe itself must change** — a new prebuild step, system package, JDK/SDK pin, `srclibs`, `rm` or `scandelete`.
- **`--enforce-lockfile` is not currently in our recipe.** `pubspec.lock.fdroid` and the `F-Droid FOSS Lockfile` CI job exist as insurance: if the recipe ever does adopt the flag, the FOSS build fails immediately (it strips `mobile_scanner` from `pubspec.yaml` while `pubspec.lock` still pins it). With the lockfile committed and CI keeping it honest, adopting the flag becomes a one-line MR adding `cp pubspec.lock.fdroid pubspec.lock` before `pub get`, rather than a diagnosis under a build that only a new tag can fix.
