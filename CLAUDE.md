...
**Localization** (`src/i18n.ts`, `src/locales/`): vue-i18n v8 with 9 locales (EN, RU, TR, BE, KA, UK, PL, ES, IT) in JSON files. **A language is wired in four places, not one**: its JSON file, `SUPPORTED_LOCALES` + the import in `src/i18n.ts`, `LanguageSelector.vue`, and the print-locale picker in `Share.vue`. `tests/unit/locales.spec.ts` fails the build if a locale is missing from any of them or its keys drift from `en.json` — added after PR #20 shipped a complete Italian translation that reached only the first two and so never appeared in a picker. Browser language auto-detected on each visit (no persistence). Slavic languages (RU, UK, BE, PL) use custom `pluralizationRules` for 3-form plurals (one|few|many). Print language is independently selectable via `printLocale` on Share.vue, passed through ShardInfo → ShardQrCode using `$t(key, locale)` 3-arg form. ShardInfo's detached Vue instance for print rendering requires explicit `i18n` injection (`new Vue({ el, i18n, render })`). All new UI strings must be added to `src/locales/en.json` (template) and all 6 translation files.
...
**Localization** (`lib/l10n/`): Flutter's official `flutter_localizations` with ARB files and code generation. 9 locales: EN, RU, TR, BE, KA, UK, PL, ES, IT. Adding one also needs `lib/widgets/language_selector.dart` (+ its test) and a `fastlane/metadata/android/<locale>/` store listing — `tools/validate_store_metadata.py` fails CI for an ARB with no listing. All UI strings use `AppLocalizations.of(context)!`. Config in `l10n.yaml`, template is `app_en.arb`. Navigation labels are built inside `build()` (not `static const`) because they need `BuildContext`.
...
**Files tab** (`lib/screens/files_screen.dart`): Scans `banana_split/` directory recursively for `.png` and `.pdf` files. Tapping a row opens the file; a row menu offers Open, Save to device, Share and Delete (with confirmation). Pull-to-refresh and empty state. Parent directory name shown as subtitle for files in subdirectories. All four actions go through `FileActionsService`, injectable via `FilesScreen(actions:)` for tests.

**File actions** (`lib/services/file_actions_service.dart`): seams over `open_filex`, `file_picker` and `share_plus`. On Android the exported files are app-private, so Open (`ACTION_VIEW`), Save to device (system save-as picker, Downloads by default, **no storage permission**) and Share are the only ways out. Every file is handed over with an explicit MIME type from a deliberately closed table (PDF, PNG). Exception: `file_picker` 8's `saveFile` takes no MIME type and sends `CREATE_DOCUMENT` with `*/*` — verified harmless on a Pixel 8 Pro (Android 17), where the Downloads provider typed a saved `.png` as `image/png` from its extension. `file_picker` 13 adds `mimeType:` (defaulting to octet-stream!) if a future target needs it. Two traps: (1) **`file_picker`'s `saveFile` ignores `bytes` on desktop** and only returns the chosen path — the service writes the file itself there, and must NOT on mobile where the result is a content URI (`pickerWritesBytes`); (2) **`open_filex` merges `READ_EXTERNAL_STORAGE` (through API 32) and `READ_MEDIA_*`** — the app's only other storage permission is camerax's `WRITE_EXTERNAL_STORAGE`, capped at API 28 — all four are stripped with `tools:node="remove"` and a `VIEW */*` query was added; `test/android_manifest_test.dart` guards both, because nothing in Dart fails when either goes missing. Check the *merged* manifest when adding any plugin; `build/app/outputs/logs/manifest-merger-release-report.txt` names the source of every permission. For the record, the GitHub build's `INTERNET`/`ACCESS_NETWORK_STATE` come from Google `datatransport` (ML Kit, via `mobile_scanner`) — absent from the F-Droid build, which strips `mobile_scanner`.
...
- Test wrapper (`tests/run_all.sh`) uses `flutter test --reporter json` piped through a Python3 parser for clean CLI output. Its total runs ~14 higher than `flutter test`'s, because it also counts the JSON reporter's per-file loading entries — quote `flutter test`'s number in commit messages.
- All new UI strings must be added to `lib/l10n/app_en.arb` (template) and all 8 translation files (`flutter gen-l10n` prints `"xx": N untranslated message(s)` for any you miss — read its output). Run `flutter gen-l10n` after editing ARB files. Use `AppLocalizations.of(context)!.keyName` in widgets.
...
- **A failed F-Droid build is pinned to its tag commit** — fixing master does nothing for it. Recovery requires a new tag, so plan on a patch release. This applies to the *app source*; a build that fails on the *recipe* while the bot's MR is still open is fixable in place on that MR, with no new tag (see !44668 below).
...
- **Version workflow**: Bump `pubspec.yaml` version+code → commit → tag → push. GitHub Actions builds the ML Kit version. **No fdroiddata MR is needed for a version bump** — F-Droid's `checkupdates` bot detects the new tag and opens the update itself, setting the commit hash. Verified: `bot: Update Banana Split to 3` landed 2026-04-09, one day after the v0.8.3 tag, with no human involvement after the initial `New app` submission. Check with:
  `gh api "repos/f-droid/fdroiddata/commits?path=metadata/com.nfcarchiver.banana_split.yml" --jq '.[]|"\(.commit.author.date[0:10])  \(.commit.author.name)  \(.commit.message|split("\n")[0])"'`
- **A quiet bot is more often a stuck MR than a missed tag.** Nothing reaches
  fdroiddata master until the MR's pipeline is green, so an unchanged metadata
  file proves nothing about detection. Look for the MR before theorising about
  `checkupdates`:
  `curl -s "https://gitlab.com/api/v4/projects/fdroid%2Ffdroiddata/merge_requests?state=all&search=Banana&in=title"`,
  then its `head_pipeline`, then
  `curl -sL https://gitlab.com/fdroid/fdroiddata/-/jobs/<JOB_ID>/raw`. The bot
  runs from a separate project, `fdroid/checkupdates-bot-fdroiddata`, one branch
  per appid; its branch head shows maintainer edits. Anonymous curl reads MRs,
  pipelines and job logs but 401s on notes — the GitLab MCP tools are
  authenticated and read and post them. Detection itself is reproducible
  locally in a scratch dir holding only `metadata/<appid>.yml` and a stub
  `config.yml`: `fdroid checkupdates -v --allow-dirty com.nfcarchiver.banana_split`
  clones the repo and prints `...updating to version X (N)`. One real trap in
  `check_tags` worth knowing: it scans only the newest **five** tags, ordered by
  `git log --tags --simplify-by-decoration` (graph order, not tag-creation
  date), so a release commit sitting low in the graph can go silently unseen.
- **The bot copies the previous build entry, it does not regenerate from `templates/build-flutter.yml`.** The bot-authored 0.8.3 entry carries our app-specific `sed -i -e '/mobile_scanner/d'` and `mv shard_scanner.dart.fdroid` lines, which appear nowhere in that template — a template-generated entry could not contain them. The consequence that holds is one-directional: a recipe change never propagates on its own, so an entry we fix only in `fdroid/*.yml` is invisible to the next bot MR, which copies whatever the last merged entry contained. The converse — that the recipe therefore cannot spontaneously acquire flags — proved false on 2026-08-03, when a maintainer added `--enforce-lockfile` to the bot's 0.9.0 MR as a diff suggestion. **Review is the third path by which the recipe changes**, alongside the bot's copy and our own MR, so an open bot MR is worth watching for edits and not only for its merge. **An MR is required only when the recipe itself must change** — a new prebuild step, system package, JDK/SDK pin, `srclibs`, `rm` or `scandelete`.
- **`--enforce-lockfile` entered the recipe on 2026-08-03, and `pubspec.lock.fdroid` is what makes it satisfiable.** Licaon_Kter added `flutter pub get --enforce-lockfile` as a diff suggestion on the bot's 0.9.0 MR ([!44668](https://gitlab.com/fdroid/fdroiddata/-/merge_requests/44668)) and applied it himself; the re-run pipeline failed in `fdroid build` at prebuild with `Would change 1 dependency` / `Unable to satisfy pubspec.yaml using pubspec.lock` — precisely the failure the second lockfile was committed to absorb. The recipe needs `cp pubspec.lock.fdroid pubspec.lock` right after the `sed` line; requested on the MR 2026-08-04. Until it lands the MR stays open, fdroiddata master still reads `CurrentVersionCode: 3`, and 0.9.0 is unpublished on F-Droid. The insurance paid off as designed: no diagnosis was needed under a build that only a new tag could fix, because the fix was already written, CI-verified at the tagged commit under the same Flutter version F-Droid reads from `release.yml`, and reduced to one line someone else can apply.
- **Fastlane changelogs** (`fastlane/metadata/android/<locale>/changelogs/<versionCode>.txt`): Named by `versionCode` (the `+N` in pubspec), not version name. Must be created for all 9 locales on each release.
